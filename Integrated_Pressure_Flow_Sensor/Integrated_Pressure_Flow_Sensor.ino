/*
  Integrated Pressure/Flow Sensor Demonstration Using Serial Port (USB CDC)

      This program integrates two sensors, a water pressure sensor and a flow sensor, 
  which can be read and communicated via a personal computer.
...
void setup(void) {
  ...
  pinMode(sigPin, INPUT);         // Initializes signal pin
...
void loop(void) {
  ...
  if ((currentTime - startTime) >= period) {  // Test whether the period has elapsed
    ...
    flowRate = abs(((-7 * pow(10, -18)) * sq(pulseCount)) + (0.5004 * pulseCount) - (8 * pow(10, -12)));
